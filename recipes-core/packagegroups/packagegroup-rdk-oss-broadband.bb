...
RDEPENDS:packagegroup-rdk-oss-broadband = "\
    ...
    dibbler-client \
    dibbler-server \
    ${@bb.utils.contains_any('DISTRO_FEATURES','kirkstone scarthgap', '', 'dhcp-client', d)} \
    ${@bb.utils.contains_any('DISTRO_FEATURES','kirkstone scarthgap', '', 'dhcp-server', d)} \
    ${@bb.utils.contains_any('DISTRO_FEATURES','kirkstone scarthgap', '', 'dhcp-server-config', d)} \
    dnsmasq \
    dropbear \
    expat \
    ez-ipupdate \
